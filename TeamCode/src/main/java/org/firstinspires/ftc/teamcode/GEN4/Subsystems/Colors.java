...
public class Colors {
    ...
    public enum BallState { EMPTY, GREEN, PURPLE, RED, BLUE }
    ...
    private void applyLED(Servo led, BallState state) {
        switch (state) {
            ...
            case BLUE: led.setPosition(0.611); break;
        }
    }
    ...
    public void updateTESTING(boolean DTAtTarget, boolean FLYWHEELatTarget, boolean DIFFYatTarget) {
        if (DTAtTarget) { applyLED(led3, BallState.BLUE); } else { applyLED(led3, BallState.RED); }
        if (FLYWHEELatTarget) { applyLED(led2, BallState.BLUE); } else { applyLED(led2, BallState.RED); }
        if (DIFFYatTarget) { applyLED(led1, BallState.BLUE); } else { applyLED(led1, BallState.RED); }
    }
